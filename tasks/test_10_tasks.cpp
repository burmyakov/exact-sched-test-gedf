...
bool find_dominating_state(const state& s, const unsigned short m, mt22& visitedStates, const unsigned priorities) {
    
    mt22::iterator itr22;
    itr22 = visitedStates.find(priorities);
    if (itr22 != visitedStates.end()) {
...
void add_state_to_map(const state& s, mt22& visitedStates, const unsigned priorities) {
    my_bitset bs_pj;
    for (int i = 0; i < s.ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    visitedStates[priorities][bs_pj][s.p[9]][s.p[8]][s.p[7]][s.p[6]][s.p[5]][s.p[4]][s.p[3]][s.p[2]][s.p[1]][s.p[0]][s.c[9]][s.c[8]][s.c[7]][s.c[6]][s.c[5]][s.c[4]][s.c[3]][s.c[2]][s.c[1]][s.c[0]] = true;
}
...
bool update_map(const state& s, const unsigned short m, mt22& visitedStates) {
...
bool test_10_tasks(const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_10tasks::populate(m_, ts_);
