...
bool find_dominating_state(const state& s, const unsigned short m, mt10& visitedStates, const unsigned priorities) {
    
    mt10::iterator itr10;
    itr10 = visitedStates.find(priorities);
    if (itr10 != visitedStates.end()) {
        ...
        for (unsigned int i9 = 0; i9 < keysNum_pj; i9++) {
            ...
            mt9::iterator itr9;
            itr9 = (itr10->second).find(bKeys_pj[i9]);
            if (itr9 != (itr10->second).end()) {
...
void add_state_to_map(const state& s, mt10& visitedStates, const unsigned priorities) {
    my_bitset bs_pj;
    for (int i = 0; i < s.ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    visitedStates[priorities][bs_pj][s.p[3]][s.p[2]][s.p[1]][s.p[0]][s.c[3]][s.c[2]][s.c[1]][s.c[0]] = true;
}
...
bool update_map(const state& s, const unsigned short m, mt10& visitedStates) {
...
bool test_4_tasks(const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_4tasks::populate(m_, ts_);
