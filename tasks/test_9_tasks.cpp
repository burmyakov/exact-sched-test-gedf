...
bool find_dominating_state(const state& s, const unsigned short m, mt20& visitedStates, const unsigned priorities) {
    
    mt20::iterator itr20;
    itr20 = visitedStates.find(priorities);
    if (itr20 != visitedStates.end()) {
        ...
        for (unsigned int i19 = 0; i19 < keysNum_pj; i19++) {
            ...
            mt19::iterator itr19;
            itr19 = (itr20->second).find(bKeys_pj[i19]);
            if (itr19 != (itr20->second).end()) {
...
void add_state_to_map(const state& s, mt20& visitedStates, const unsigned priorities) {
    my_bitset bs_pj;
    for (int i = 0; i < s.ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    visitedStates[priorities][bs_pj][s.p[8]][s.p[7]][s.p[6]][s.p[5]][s.p[4]][s.p[3]][s.p[2]][s.p[1]][s.p[0]][s.c[8]][s.c[7]][s.c[6]][s.c[5]][s.c[4]][s.c[3]][s.c[2]][s.c[1]][s.c[0]] = true;
}
...
bool update_map(const state& s, const unsigned short m, mt20& visitedStates) {
...
bool test_9_tasks(const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_9tasks::populate(m_, ts_);
