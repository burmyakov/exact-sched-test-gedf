...
bool find_dominating_state(const state& s, const unsigned short m, mt16& visitedStates, const unsigned priorities) {
    
    mt16::iterator itr16;
    itr16 = visitedStates.find(priorities);
    if (itr16 != visitedStates.end()) {
        ...
        for (unsigned int i15 = 0; i15 < keysNum_pj; i15++) {
            ...
            mt15::iterator itr15;
            itr15 = (itr16->second).find(bKeys_pj[i15]);
            if (itr15 != (itr16->second).end()) {
...
void add_state_to_map(const state& s, mt16& visitedStates, const unsigned priorities) {
    my_bitset bs_pj;
    for (int i = 0; i < s.ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    visitedStates[priorities][bs_pj][s.p[6]][s.p[5]][s.p[4]][s.p[3]][s.p[2]][s.p[1]][s.p[0]][s.c[6]][s.c[5]][s.c[4]][s.c[3]][s.c[2]][s.c[1]][s.c[0]] = true;
}
...
bool update_map(const state& s, const unsigned short m, mt16& visitedStates) {
...
bool test_7_tasks(const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_7tasks::populate(m_, ts_);
