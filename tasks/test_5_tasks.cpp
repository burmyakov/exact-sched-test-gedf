...
bool find_dominating_state(const state& s, const unsigned short m, mt12& visitedStates, const unsigned priorities) {
    
    mt12::iterator itr12;
    itr12 = visitedStates.find(priorities);
    if (itr12 != visitedStates.end()) {
        ...
        for (unsigned int i11 = 0; i11 < keysNum_pj; i11++) {
            ...
            mt11::iterator itr11;
            itr11 = (itr12->second).find(bKeys_pj[i11]);
            if (itr11 != (itr12->second).end()) {
...
void add_state_to_map(const state& s, mt12& visitedStates, const unsigned priorities) {
    my_bitset bs_pj;
    for (int i = 0; i < s.ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    visitedStates[priorities][bs_pj][s.p[4]][s.p[3]][s.p[2]][s.p[1]][s.p[0]][s.c[4]][s.c[3]][s.c[2]][s.c[1]][s.c[0]] = true;
}
...
bool update_map(const state& s, const unsigned short m, mt12& visitedStates) {
...
bool test_5_tasks(const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_5tasks::populate(m_, ts_);
